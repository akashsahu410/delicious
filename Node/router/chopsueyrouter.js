...
const userapi=require('../api/chopsueyapi')

app.post('/get',async (req,res)=>{
    try{
        console.log("inside get of chopsuey ")
        let data=await userapi.finddata()
        console.log("data get from api in router",data)
        res.status(200).send(data)
    }
    catch(err){
        console.log("error in chopsuey router",err)
...
app.post('/delete',async (req,res)=>{
    try{
        console.log("inside delete of chopsuey ",req.body)
...
app.post('/change',async (req,res)=>{
    try{
        console.log("inside change of chopsuey ",req.body)
